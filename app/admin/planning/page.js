...
export default function PlanningPage() {
  ...
  const TYPES = { cours: { label: 'Cours', color: '#a78bfa' }, kholle: { label: 'Khôlle', color: '#34d399' }, entretien: { label: 'Entretien', color: '#f87171' } }
  ...
  const s = {
    ...
    td: { padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,0.05)', fontSize: '13px', color: '#9e9c96', verticalAlign: 'top' },
    ...
    modalBox: { background: '#18181c', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '16px', padding: '28px', width: '100%', maxWidth: '560px' },
    ...
    select: { width: '100%', padding: '9px 12px', background: '#1e1e24', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#e8e6e0', fontSize: '13px', outline: 'none', boxSizing: 'border-box' },
    grid2: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' },
    checkList: { background: '#1e1e24', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', padding: '8px', maxHeight: '160px', overflowY: 'auto' },
    checkItem: { display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 8px', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', color: '#9e9c96' },
  }
              ...
              <tbody>
                {seances.map(s2 => (
                  <tr key={s2.id}>
                    ...
                    <td style={{ ...s.td, color: '#e8e6e0', fontWeight: '500' }}>
                      {s2.titre}
                      {s2.matiere && <div style={{ fontSize: '11px', color: '#6e6c66', marginTop: '2px' }}>{s2.matiere}</div>}
                    </td>
                    <td style={s.td}>
                      {new Date(s2.date_debut).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short' })}
                      <div style={{ fontSize: '11px', color: '#6e6c66', marginTop: '2px' }}>
                    ...
                    <td style={s.td}>
                      <span style={{
                        ...
                        color: s2.statut === 'effectuee' ? '#34d399' : s2.statut === 'annulee' ? '#f87171' : '#fbbf24'
                      }}>
            ...
            <form onSubmit={createSeance}>
              ...
              <label style={s.label}>
                ...
                <button type="button" onClick={() => toggleAll('prof_ids', profs)}
                  style={{ marginLeft: '8px', fontSize: '10px', color: '#a78bfa', background: 'none', border: 'none', cursor: 'pointer' }}>
                  {form.prof_ids.length === profs.length ? 'Tout désélectionner' : 'Tout sélectionner'}
                </button>
              </label>
              <div style={s.checkList}>
                {profs.length === 0 ? <div style={{ color: '#6e6c66', fontSize: '12px', padding: '8px' }}>Aucun prof actif</div> :
                  profs.map(prof => (
                    <div key={prof.id} style={{
                      ...s.checkItem,
                      background: form.prof_ids.includes(prof.id) ? 'rgba(167,139,250,0.1)' : 'none',
                      color: form.prof_ids.includes(prof.id) ? '#a78bfa' : '#9e9c96'
                    }}
              ...
              <div style={s.checkList}>
                {eleves.length === 0 ? <div style={{ color: '#6e6c66', fontSize: '12px', padding: '8px' }}>Aucun élève actif</div> :
                  eleves.map(eleve => (
                    <div key={eleve.id} style={{
                      ...s.checkItem,
                      background: form.eleve_ids.includes(eleve.id) ? 'rgba(167,139,250,0.1)' : 'none',
                      color: form.eleve_ids.includes(eleve.id) ? '#a78bfa' : '#9e9c96'
                    }}
