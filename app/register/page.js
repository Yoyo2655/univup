...
export default function RegisterPage() {
  ...
  async function handleRegister(e) {
    ...
    if (!cgvAccepted) { setError('Tu dois accepter les CGV pour continuer'); return }
    setLoading(true)
    setError('')
    const { data, error: signUpError } = await supabase.auth.signUp({ email, password })
    if (signUpError) { setError(signUpError.message); setLoading(false); return }
    const { error: dbError } = await supabase.from('users').insert({ id: data.user.id, email, full_name: fullName, role, is_active: false })
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', background: '#0e0d0d', fontFamily: "'DM Sans', system-ui, sans-serif", position: 'relative', overflow: 'hidden' }}>
      ...
      <div className="reg-right" style={{ flex: 1, display: 'flex', justifyContent: 'center', position: 'relative', overflowY: 'auto' }}>
        ...
        <div style={{ width: '100%', maxWidth: '360px', position: 'relative', zIndex: 1, paddingTop: '20px', paddingBottom: '20px' }}>
          <h2 style={{ fontSize: '26px', fontWeight: '700', color: '#f0eeea', letterSpacing: '-0.5px', marginBottom: '6px' }}>Creer mon compte</h2>
          ...
          <form onSubmit={handleRegister}>
            <div style={{ marginBottom: '16px' }}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                {['eleve', 'prof'].map(r => (
                  <div key={r} onClick={() => setRole(r)} style={{ flex: 1, padding: '12px', textAlign: 'center', borderRadius: '8px', cursor: 'pointer', fontSize: '14px', fontWeight: '500', border: role === r ? '1px solid rgba(155,142,196,0.5)' : '1px solid rgba(255,255,255,0.08)', background: role === r ? 'rgba(155,142,196,0.1)' : 'rgba(255,255,255,0.04)', color: role === r ? '#9b8ec4' : '#6e6c66', transition: 'all 0.15s' }}>
