...
export default function AbonnementPage() {
  ...
  async function demanderChangementPack(pack) {
    ...
    await supabase.from('messages').insert({
      user_id: user.id,
      contenu: 'Je souhaite changer mon abonnement actuel (' + abonnement.pack_nom + ') pour le pack : ' + pack.nom + ' (' + pack.prix + 'EUR / ' + pack.duree_mois + ' mois). Merci de traiter ma demande.'
    })
  ...
  if (!abonnement) return (
    <div style={{ color: t.text }}>
      ...
      <div style={s.content}>
        ...
        {packsDisponibles.length === 0 ? (
          <div style={{ color: t.muted, textAlign: 'center', padding: '40px' }}>
            Aucun pack disponible — contacte UnivUp.
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '16px' }}>
            {packsDisponibles.map(pack => (
              <div
                ...
                style={{ background: t.surface, border: '1px solid ' + t.border, borderRadius: '12px', padding: '24px', cursor: 'pointer' }}
              ...
              >
                ...
                {pack.description && <div style={{ fontSize: '13px', color: t.muted2, marginBottom: '16px', lineHeight: 1.6 }}>{pack.description}</div>}
      ...
      {packSelectionne && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }} onClick={() => setPackSelectionne(null)}>
          <div style={{ background: t.surface, border: '1px solid ' + t.border2, borderRadius: '16px', padding: '32px', width: '100%', maxWidth: '480px' }} onClick={e => e.stopPropagation()}>
            <div style={{ marginBottom: '24px' }}>
              <div style={{ fontSize: '12px', color: t.muted, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '6px' }}>Pack selectionne</div>
            ...
            <div style={{ fontSize: '13px', color: t.muted, marginBottom: '20px', lineHeight: 1.6 }}>
              Pour souscrire, effectue un virement bancaire avec les informations ci-dessous et contacte UnivUp pour confirmer.
            </div>
            {[
            ...
            <button onClick={() => setPackSelectionne(null)} style={{ width: '100%', marginTop: '16px', padding: '10px', background: 'none', border: '1px solid ' + t.border2, borderRadius: '8px', color: t.muted2, cursor: 'pointer', fontSize: '13px' }}>
              Fermer
            </button>
          </div>
        </div>
  ...
  return (
    <div style={{ color: t.text }}>
        ...
        <div style={s.grid2}>
          <div style={s.card}>
            ...
            <div style={s.cardBody}>
              <div style={{ fontSize: '22px', fontWeight: '600', color: t.purple, marginBottom: '4px' }}>{abonnement.pack_nom}</div>
              ...
              <div style={s.row}><span style={s.label}>Montant total</span><span style={s.value}>{abonnement.montant}€</span></div>
              <div style={s.row}><span style={s.label}>Deja verse</span><span style={{ ...s.value, color: t.teal }}>{totalVerse.toFixed(2)}€</span></div>
              <div style={{ ...s.row, borderBottom: 'none' }}>
                <span style={s.label}>Reste a payer</span>
                <span style={{ ...s.value, color: resteAPayer > 0 ? t.amber : t.teal }}>
                  {resteAPayer > 0 ? resteAPayer.toFixed(2) + '€' : 'Solde'}
                </span>
              </div>
              <button onClick={() => setShowChangePack(true)} style={{ width: '100%', marginTop: '16px', padding: '8px', background: 'none', border: '1px solid ' + t.border2, borderRadius: '8px', color: t.muted2, cursor: 'pointer', fontSize: '12px' }}>
                Changer de pack →
              </button>
            </div>
          </div>
          ...
          <div style={s.card}>
            ...
            <div style={s.cardBody}>
              <div style={{ fontSize: '12px', color: t.muted, marginBottom: '16px', lineHeight: 1.6 }}>
                Pour effectuer ton paiement, realise un virement avec les informations ci-dessous.
                Indique bien ta <strong style={{ color: t.text }}>reference unique</strong> dans le motif.
              </div>
              {[
                { label: 'Beneficiaire', value: 'YONI MILO ATTAL', mono: false },
                { label: 'IBAN', value: 'FR76 2823 3000 0191 3356 4211 372', mono: true },
              ...
              <div style={s.ribBox}>
                <div style={{ fontSize: '10px', color: t.muted, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '8px' }}>Reference a indiquer</div>
                <div style={{ fontSize: '16px', fontWeight: '600', color: t.purple, fontFamily: 'monospace', letterSpacing: '0.05em' }}>{abonnement.reference_virement || '-'}</div>
              </div>
              <div style={{ fontSize: '11px', color: t.muted, marginTop: '12px' }}>Une fois le virement recu, ton acces sera active sous 24h.</div>
            </div>
          </div>
        </div>
              ...
              <thead>
                <tr>
                  {['Date', 'Montant', 'Statut'].map(h => (
                    <th key={h} style={{ textAlign: 'left', fontSize: '10px', fontWeight: '500', color: t.muted, textTransform: 'uppercase', letterSpacing: '0.06em', padding: '10px 20px', borderBottom: '1px solid ' + t.border }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {paiements.map(p => (
                  <tr key={p.id}>
                    ...
                    <td style={{ padding: '12px 20px', fontSize: '13px', fontWeight: '600', color: t.teal, fontFamily: 'monospace', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>+{p.montant}€</td>
                    <td style={{ padding: '12px 20px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                      <span style={{ padding: '2px 8px', borderRadius: '20px', fontSize: '11px', fontWeight: '500', background: 'rgba(52,211,153,0.12)', color: t.teal }}>Recu</span>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {showChangePack && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }} onClick={() => setShowChangePack(false)}>
          <div style={{ background: t.surface, border: '1px solid ' + t.border2, borderRadius: '16px', padding: '32px', width: '100%', maxWidth: '560px', maxHeight: '80vh', overflow: 'auto' }} onClick={e => e.stopPropagation()}>
            <h2 style={{ fontSize: '16px', fontWeight: '600', color: t.text, marginBottom: '6px' }}>Changer de pack</h2>
            <p style={{ fontSize: '13px', color: t.muted, marginBottom: '20px', lineHeight: 1.6 }}>
              Choisis ton nouveau pack. Une demande sera envoyee automatiquement dans le chat.
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {packsDisponibles.map(pack => (
                <div
                  key={pack.id}
                  onClick={() => demanderChangementPack(pack)}
                  style={{ background: t.surface2, border: '1px solid ' + t.border, borderRadius: '10px', padding: '16px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}
                  onMouseEnter={e => e.currentTarget.style.borderColor = t.purple}
                  onMouseLeave={e => e.currentTarget.style.borderColor = t.border}
                >
                  <div>
                    <div style={{ fontSize: '14px', fontWeight: '500', color: t.text, marginBottom: '4px' }}>{pack.nom}</div>
                    {pack.description && <div style={{ fontSize: '12px', color: t.muted }}>{pack.description}</div>}
                  </div>
                  <div style={{ textAlign: 'right', flexShrink: 0, marginLeft: '16px' }}>
                    <div style={{ fontSize: '18px', fontWeight: '700', color: t.purple }}>{pack.prix}€</div>
                    <div style={{ fontSize: '11px', color: t.muted }}>{pack.duree_mois} mois</div>
                  </div>
                </div>
              ))}
            </div>
            <button onClick={() => setShowChangePack(false)} style={{ width: '100%', marginTop: '16px', padding: '10px', background: 'none', border: '1px solid ' + t.border2, borderRadius: '8px', color: t.muted2, cursor: 'pointer', fontSize: '13px' }}>
              Annuler
            </button>
          </div>
        </div>
